...
class projectModel extends baseModel {
  ...
  getBaseInfo(id, select) {
    select =
      select ||
      '_id uid name basepath switch_notice desc group_id project_type env icon color add_time up_time pre_script after_script project_mock_script is_mock_open strice is_json5 tag doc_address robot';
    return this.model
      .findOne({
        _id: id
      })
