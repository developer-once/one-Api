...
// const { oneApi } = require("../oneAPI/index");
...
class projectController extends baseController {
   ...
   * @alias OneAPI add project API
   */
  async add(ctx) {
    let params = ctx.params;
    // ------ OneAPi.3 获取用户 cookie ------
    let user_cookie = ctx.request.headers.cookie;
    let cookie = "";
    let user_uid = "";
    user_cookie.split("; ").map((item, index) => {
      if (item.indexOf("_yapi_token") > -1) {
        cookie = item.split("=")[1];
      } else if (item.indexOf("_yapi_uid") > -1) {
        user_uid = item.split("=")[1];
      }
    })
    ...
    // ---- OneAPI-add ----
    if (params.doc_address) {
      child_process.exec(`node server/oneAPI/index.js --doc_address=${params.doc_address} --is_new=${true} --cookie=${cookie} --uid=${user_uid} --project_id=${result._id}`, () => {
